...
if "day_adjustments" not in st.session_state:
    st.session_state.day_adjustments = {d: 0 for d in DAY_ORDER}
...
if page == "📊 Sales Dashboard":
    ...
    if st.session_state.weekly_data:
        ...
        typical_weights = {
            "Monday": 0.10, "Tuesday": 0.11, "Wednesday": 0.12,
            "Thursday": 0.13, "Friday": 0.18, "Saturday": 0.22, "Sunday": 0.14
        }
        avg_revenue = avg_weekly_revenue

        # Build day projections
        day_proj = {}
        ...
        if st.button(f"🔢 Calculate {selected_vendor} Order", type="primary",
            ...
            if not vendor_ingredients:
                ...
                for ingredient, data in vendor_ingredients.items():
                    ...
                    total_order = 0
                    for day in DAY_ORDER:
                        # Daily estimate = weekly total × day weight
